...
def find_todays_videos(channel_id=None, keyword=None):
    """
    RSS 피드에서 오늘 날짜의 '모닝루틴' 영상을 검색합니다.
    ...
    print(f"[디버그] RSS 피드 URL: {url}")
    
    try:
        response = requests.get(url, timeout=15)
        print(f"[디버그] RSS 응답 코드: {response.status_code}")
        
        if response.status_code != 200:
            print(f"❌ RSS 피드 응답 오류: {response.status_code}")
            return []
        ...
        print(f"[디버그] 발견된 영상 수: {len(entries)}")
        
        today_str = datetime.now().strftime("%Y%m%d")
        print(f"[디버그] 오늘 날짜 기준: {today_str}")
        ...
        for entry in entries:
            ...
            if today_str in title:
                ...
                print(f"[디버그] ✅ 오늘자 영상 발견: {title} ({video_date})")
            else:
                print(f"[디버그] ❌ 날짜 불일치 (Skip): {title} (ID: {video_id})")
...
def extract_transcript(video_id):
    ...
    try:
        ...
        try:
            ...
            print("한국어 자막 없음, 자동 생성 자막 검색...")
            try:
                transcript = transcript_list.find_generated_transcript(['ko'])
            except Exception:
                print("한국어 자동 생성 자막도 없음.")
                return None
