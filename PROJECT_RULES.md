...
- **로그 관리**:
  - `data/trade/status.json`: 최근 거래 기록 **1,000건** 유지 (FIFO 방식)
  - `logs/`: 날짜별 로그 자동 로테이션 (30일 보관)
...
- 매매 현황: `/trade.html` 링크
- **새로 추가되는 모듈은 메인 페이지 사이드바에 진입로가 추가되어야 합니다.**
- 활성 페이지 항목은 강조 표시
...
**상단 로고 규칙**:
- 왼쪽 상단에 위치한 로고(또는 프로젝트명)를 클릭 시 항상 메인 페이지(`/`)로 돌아올 수 있도록 링크가 적용되어야 합니다.
